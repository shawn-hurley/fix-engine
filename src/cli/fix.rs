...
pub async fn run(opts: FixOpts) -> Result<()> {
    ...
    // Build fix context registry (generic — no framework-specific contexts)
    ...
    if let Some(ref rules_strategies_path) = opts.rules_strategies {
        ...
        match fix_engine_core::load_strategies_and_families(rules_strategies_path) {
            Ok((strats, families)) => {
    ...
    let mut plan = engine::plan_fixes(&output, &project, &merged_strategies, &lang)?;
